...
def main():
    # 确保输出目录存在
    os.makedirs(PathConfig.CONTENT_VALIDATOR_AUTO_OUTPUT, exist_ok=True)
    
    # 处理每个json文件
    for filename in os.listdir(PathConfig.CONTENT_VALIDATOR_AUTO_INPUT):
        if filename.endswith('.json'):
            source_path = os.path.join(PathConfig.CONTENT_VALIDATOR_AUTO_INPUT, filename)
            target_path = os.path.join(PathConfig.CONTENT_VALIDATOR_AUTO_OUTPUT, filename)
