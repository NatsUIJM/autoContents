...
def process_pdf_with_bookmarks():
    ...
    if len(json_files) != 1:
        print(f"错误：找到了 {len(json_files)} 个_final.json 文件，期望只有 1 个")
        return
    ...
    if len(pdf_files) != 1:
        print(f"错误：找到了 {len(pdf_files)} 个 PDF 文件，期望只有 1 个")
        return
    ...
    if len(info_json_files) != 1:
        print(f"错误：找到了 {len(info_json_files)} 个 info JSON 文件，期望只有 1 个")
        return
    ...
    try:
        ...
        # toc_structure 字段不再用于过滤逻辑，但仍可读取以防后续扩展
        ...
        toc_entry = {
            'text': '目录',
            'number': toc_start,  # 原来的 toc_start 是当前页，+1 就是下一页
            'level': 1
        }
        ...
        for item in toc_data:
            try:
                if item['text'] != '目录':  # 不调整"目录"条目的页码偏移逻辑保持原样，但不再跳过特定模式
        ...
        def create_bookmark_tree(items):
            ...
            for item in items:
                try:
                    # 检查页码是否有效（页码从 1 开始，pdf.pages 索引从 0 开始）
