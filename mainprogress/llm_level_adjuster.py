...
class PatternMatcher:
    def __init__(self, patterns: Dict[int, List[str]]):
        ...
        self.max_level = max(patterns.keys()) if patterns else 0
...
def prepare_iteration_input(original_json: str, current_patterns: dict, unmatched_titles: List[str]) -> str:
    return json.dumps({
        "original_structure": json.loads(original_json),
        "current_patterns": current_patterns,
        "unmatched_titles": unmatched_titles
    }, ensure_ascii=False, indent=2)

async def is_suitable_for_regex(client, model_name: str, items: List[dict]) -> bool:
...
async def process_file(client, model_name: str, file_path: Path, output_dir: Path, cache_dir: Path):
    try:
        ...
        if not await is_suitable_for_regex(client, model_name, original_data):
...
async def main():
    ...
    input_dir = Path(os.getenv("LEVEL_ADJUSTER_INPUT", "input"))
    output_dir = Path(os.getenv("LEVEL_ADJUSTER_OUTPUT", "output"))
    cache_dir = Path(os.getenv("LEVEL_ADJUSTER_CACHE", "cache"))
    ...
    for file_path in input_dir.glob("*.json"):
        tasks.append(process_file(client, model_name, file_path, output_dir, cache_dir))

    if tasks:
        await asyncio.gather(*tasks)
    else:
        print(f"未在 {input_dir} 找到任何 JSON 文件。")
