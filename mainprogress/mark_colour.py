...
def setup_logger(output_dir):
    ...
    try:
        ...
        log_file = os.path.join(output_dir, f"mark_color_{timestamp}.log")
        
        # 配置日志记录器
        logger = logging.getLogger("mark_color")
        ...
        # 如果日志设置失败，创建一个简单的控制台记录器
        print(f"日志设置失败: {e}")
        fallback_logger = logging.getLogger("mark_color_fallback")
...
def put_chinese_text(img, text, position, color):
    ...
    try:
        ...
        if font is None:
            logger.warning("未能加载任何中文字体，跳过文字绘制")
    ...
    try:
        ...
        try:
            if image_path and os.path.exists(image_path):
                ...
                if image is None:
                    logger.warning(f"图像文件存在但无法读取: {image_path}")
        except Exception as e:
        ...
        try:
            ...
            logger.error(f"无法读取JSON文件: {json_path}, 错误: {e}")
        ...
        try:
            ...
            if image is not None:
                ...
                for line in lines:
                    ...
                    try:
                        ...
                        logger.error(f"绘制文本框失败: {e}")
                
                # 保存图像
                try:
                    cv2.imwrite(output_image_path, image)
                    logger.info(f"已保存图像: {output_image_path}")
                except Exception as e:
                    logger.error(f"保存图像失败: {output_image_path}, 错误: {e}")
            else:
                logger.info("图像处理已跳过，只生成JSON输出")
            ...
            logger.info(f"已保存JSON: {output_json_path}")
            
        except Exception as e:
            logger.error(f"标记处理失败，将输出全绿JSON: {e}")
            logger.error(traceback.format_exc())
            
            # 在失败时输出全绿的JSON
            with open(output_json_path, 'w', encoding='utf-8') as f:
                json.dump(default_output_data, f, ensure_ascii=False, indent=2)
            logger.info(f"已保存全绿备用JSON: {output_json_path}")
            
    except Exception as e:
        logger.error(f"处理失败: {e}")
        logger.error(traceback.format_exc())
        
        # 在最坏情况下，尝试从原始JSON构建一个全绿的输出
        try:
            with open(json_path, 'r', encoding='utf-8') as f:
                data = json.load(f)
            ...
            with open(output_json_path, 'w', encoding='utf-8') as f:
                json.dump(fallback_output, f, ensure_ascii=False, indent=2)
            logger.info(f"已保存应急全绿JSON: {output_json_path}")
            
        except Exception as final_e:
            logger.critical(f"无法创建应急输出: {final_e}")
        ...
        for filename in os.listdir(input_dir):
            if filename.endswith('_raw_response.json'):
                try:
                    ...
                    if not image_path:
                        logger.warning(f"找不到对应的图像文件: {base_name}.*")
                        # 尝试查找PDF页面格式
...
if __name__ == "__main__":
    try:
        ...
        # 记录初始信息
        logger.info("==== mark_color.py 开始执行 ====")
        
        # 检查环境变量是否存在
        if not input_dir:
            logger.error("环境变量 MARK_COLOR_INPUT 未设置")
            sys.exit(1)
        if not input_image_dir:
            logger.error("环境变量 MARK_COLOR_INPUT_IMAGE 未设置")
            sys.exit(1)
        if not output_dir:
            logger.error("环境变量 MARK_COLOR_OUTPUT 未设置")
            sys.exit(1)
            
        logger.info(f"输入目录: {input_dir}")
        logger.info(f"图像目录: {input_image_dir}")
        logger.info(f"输出目录: {output_dir}")
        
        # 记录jieba可用性
        if jieba_available:
            logger.info("jieba分词模块已加载")
        else:
            logger.warning("未能导入jieba分词模块，相关功能将被跳过")
            
        process_directory(input_dir, input_image_dir, output_dir)
        logger.info("==== mark_color.py 执行完成 ====")
    except Exception as e:
        if logger:
            logger.critical(f"执行脚本时出错: {e}")
            logger.critical(traceback.format_exc())
        else:
            # 如果logger还未初始化
            print(f"初始化日志记录器前出错: {e}")
            traceback.print_exc()