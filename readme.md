...
在 95% 以上的测试样本中，阿里云能够提供稳定且优质的服务，足以满足大多数需求。因此建议优先完成阿里云相关服务的申请。仅在生成结果严重低于预期时，再考虑使用 Azure 作为补充方案。[点此查看全部教程](./docs/如何申请云服务账户.md)。
...
注：若出现闪退现象，请检查当前用户是否具有管理员权限；若提示`系统找不到指定的路径`，请将文件夹移动到不含有中文等特殊字符的目录，然后再试一次。
...
注：如果处理结果文件非常小且无法打开，请进入项目目录`data`文件夹中查看处理记录。每个处理记录文件夹下的`input_pdf`文件夹存储了原始 PDF 文件，可以通过这种方式找到对应的处理记录。将相关处理记录打包发送至`uijm2004@outlook.com`以获取解决方案。

## 疑难解答与问题反馈

请联系 QQ`929849897`或邮箱`uijm2004@outlook.com`提交问题反馈。
