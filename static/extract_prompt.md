...
4. 不要遗漏任何目录项目，存在页码的条目必然是一个目录条目，而与它的具体内容无关。但是需要忽略"前言"或"第x版前言"这种与正文一点关系都没有的条目，有一些条目没有直接显示页码，并不代表它们不是目录条目，请基于具体语义而非是否有规范的格式来判断一个内容是否是目录的一部分。如果出现大批量的没有页码的目录，例如用于描述某个章节下都有哪些关键词的内容，则需要忽略。
...
10. 在提取标题时，要忠于原文，严禁擅自对标题进行增添删改。
11. 禁止使用带圈的数字，例如①，而应该使用常见的数字，例如1。